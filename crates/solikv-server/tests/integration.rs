                ...
                fn to_frame(r: CommandResponse) -> RespFrame {
                    match r {
                        ...
                        CommandResponse::Array(items) => {
                            RespFrame::Array(items.into_iter().map(to_frame).collect())
                        }
...
async fn test_getrange() {
    ...
    let result: String = redis::cmd("GETRANGE")
        .arg("mykey")
        .arg(0)
        .arg(4)
        .query_async(&mut con)
...
async fn test_subscribe_message_delivery() {
    ...
    let mut sub = TcpStream::connect(format!("127.0.0.1:{}", port))
        .await
        .unwrap();
    // Publisher connection
    let mut pub_conn = TcpStream::connect(format!("127.0.0.1:{}", port))
        .await
        .unwrap();
    ...
    // Publish a message
    let _pub_responses =
        send_raw_command(&mut pub_conn, &["PUBLISH", "test-ch", "hello-world"]).await;
...
async fn test_keyspace_notification_on_set() {
    ...
    let mut sub = TcpStream::connect(format!("127.0.0.1:{}", port))
        .await
        .unwrap();
    let mut cmd_conn = TcpStream::connect(format!("127.0.0.1:{}", port))
        .await
        .unwrap();

    // Enable all keyspace notifications via CONFIG SET
    let _ = send_raw_command(
        &mut cmd_conn,
    ...
    assert!(
        !messages.is_empty(),
        "Expected to receive a keyevent notification"
    );
...
async fn test_keyspace_notification_disabled_by_default() {
    ...
    assert!(
        messages.is_empty(),
        "Expected no notifications when disabled"
    );
}
